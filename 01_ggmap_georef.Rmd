...
The examples below documents ggmap syntax, starting with google basemaps as examples.  **HOWEVER**, the last examples, **stamen maps, work best.**  This is because 2018 changes in the Google Maps API render ggmap for google maps far less convenient than was previously when generating quick static maps.  **None the less**, using the `ggmap` with *stamen* map features is still an excellent way to generate high contrast black and white maps that are quite useful for printed publications.
